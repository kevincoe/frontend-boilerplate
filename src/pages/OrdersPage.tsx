...
export const OrdersPage: React.FC = () => {
  ...
  const handleFinishOrder = async (order: OrderResponse) => {
    if (
      !window.confirm(
        "Tem certeza que deseja finalizar esta locação e devolver os equipamentos para o estoque?",
      )
    )
      return;
    try {
      ...
      await loadOrders();
      alert(
        "Locação finalizada com sucesso. Equipamentos retornados ao estoque.",
      );
    } catch (err) {
      const msg = axios.isAxiosError(err)
        ? err.response?.data?.message || err.response?.data?.error
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Gerencie seus pedidos e orçamentos
            </p>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6">
                Tem certeza que deseja excluir este pedido? Orçamentos e Pedidos
                Finalizados podem ser apagados permanentemente. Pedidos em
                andamento devem ser finalizados antes.
              </p>
